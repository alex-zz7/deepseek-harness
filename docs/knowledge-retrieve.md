...
同一句，本机热身后 5 次中位数（2026-09-13）：

| | 检索墙钟 | 注入正文 | 估 token | 文件 |
|---|---|---|---|---|
| 先认文档 `retrieve` | 37 ms | 4278 字 | ~1097 | 仅 `gptimage2/SKILL.md`（6 段） |
| 旧平铺 `search` | 36 ms | 6113 字 | ~1567 | 8 个文件（含 yt-story-script、follow-builders、proma-coach 等） |

检索本身几乎一样快：都要约 35–40 ms，外加一次问句 embedding。变快的不是近邻扫描，是少塞错文、少追问。

对话里的用量是整轮生成，不是检索延迟。同一句的真实会话：

| 路径 | 轮次 | 用时 | 用量 | 结果 |
|---|---|---|---|---|
| 旧平铺检索 | 第 1 轮 | 6 秒 | 35.9K tok | 「本库没有讲原理」 |
| 旧平铺检索 | 再追问到第 4 轮 | 累计 LLM ~32 秒 | 40.1K → 44.6K → 49.8K | 才承认正文一直在 `gptimage2/SKILL.md` |
| 先认文档 | 第 1 轮 | 6 秒 | ~36K tok | 直接讲异步任务、轮询、参数映射 |
| 浏览器（不走这套检索） | 1 轮 12 步工具 | LLM 29 秒 + 工具 45 秒 | 输出 5.5K tok | 自己翻磁盘才读到本机 `SKILL.md` |

第一轮的 6 秒 / ~36K 差不多：系统提示和工具表才是大头。旧路把这 6 秒花在答错上，还要再烧三轮；浏览器则把将近一分钟花在读文件。
