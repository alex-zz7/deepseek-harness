  ...
  switch (os.platform()) {
    case 'darwin':
      return [
        ...
        { id: 'quark',         label: 'Quark',          devToolsPath: path.join(home, 'Library/Application Support/Quark/DevToolsActivePort') },
      ];
    case 'linux':
      return [
...
async function detectAll() {
  ...
  // Quark 用 --remote-debugging-port 启动时可能不写 DevToolsActivePort
  if (!result.some(b => b.id === 'quark') && await checkPort(9222)) {
    const quark = knownBrowsers().find(b => b.id === 'quark');
    if (quark) {
      let wsPath = null;
      try {
        const info = await fetch('http://127.0.0.1:9222/json/version', { signal: AbortSignal.timeout(1500) }).then(r => r.json());
        const ws = info?.webSocketDebuggerUrl || '';
        const idx = ws.indexOf('/devtools/');
        if (idx !== -1) wsPath = ws.slice(idx);
      } catch { /* 仍可用默认路径 */ }
      result.push({ ...quark, port: 9222, wsPath });
    }
  }
  return result;
}
