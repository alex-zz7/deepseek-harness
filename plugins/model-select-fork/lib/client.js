	...
	factory: (require) => {
		...
		*/
		function ModelSelect({ locked, available, directory, load, select, t }) {
			...
			const flyoutRef = (0, react.useRef)(null);
			const [menuPos, setMenuPos] = (0, react.useState)(null);
			const flyoutMode = typeof window !== "undefined" && window.__DSH_SIDEBAR_ACTIONS__ === true;
			...
			(0, react.useLayoutEffect)(() => {
				...
				const place = () => {
					...
					const MARGIN = 12;
					const lw = flyoutMode ? menuRef.current?.offsetWidth ?? 0 : Math.max(menuRef.current?.offsetWidth ?? 0, flyoutRef.current?.offsetWidth ?? 0);
					const lh = flyoutMode ? menuRef.current?.offsetHeight ?? 0 : Math.max(menuRef.current?.offsetHeight ?? 0, flyoutRef.current?.offsetHeight ?? 0);
					let x = rect.right - lw;
					let y = rect.top - 8 - lh;
			...
			const onBlur = (event) => {
				if (event.relatedTarget instanceof Node && (rootRef.current?.contains(event.relatedTarget) === true || menuRef.current?.contains(event.relatedTarget) === true || flyoutRef.current?.contains(event.relatedTarget) === true)) return;
				close();
			};
			...
			const flyoutCloseTimer = (0, react.useRef)(null);
			...
			const flyoutStyle = (() => {
				if (menuPos === null) return MEASURE_STYLE;
				// Anchored to the main menu, never pushed around by the viewport:
				// the flyout shares the menu's bottom edge (or its top edge when
				// there is more room below) and sits a 4px gap to its right, so
				// the two panels stay visually attached and the pointer travels
				// the shortest possible distance. Only the height is capped —
				// a too-tall list scrolls instead of moving away.
				const MARGIN = 12;
				const GAP = 4;
				...
				const menuRight = menuPos.left + mw;
				const menuBottom = menuPos.top + mh;
				const estW = Math.min(320, iw - 2 * MARGIN);
				const style = menuRight + GAP + estW <= iw - MARGIN ? {
					left: menuRight + GAP
				} : {
					right: Math.max(MARGIN, iw - menuPos.left + GAP)
				};
				const spaceAbove = menuBottom - MARGIN;
				const spaceBelow = ih - MARGIN - menuPos.top;
				if (spaceAbove >= spaceBelow) {
					style.bottom = Math.max(MARGIN, ih - menuBottom);
					style.maxHeight = Math.min(360, spaceAbove);
				} else {
					style.top = menuPos.top;
					style.maxHeight = Math.min(360, spaceBelow);
				}
				return style;
			})();
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					open && (0, react_dom.createPortal)((0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						(flyoutMode || pane === "root") && (0, react_jsx_runtime.jsx)("div", {
							ref: menuRef,
							id: `${id}-menu`,
							...
							onMouseEnter: flyoutMode ? () => {
								clearFlyoutClose();
							} : void 0,
							onMouseLeave: flyoutMode ? () => {
								scheduleFlyoutClose();
							} : void 0,
							children: [
					...
					pane !== "root" && (0, react_jsx_runtime.jsx)("div", {
						ref: flyoutRef,
						className: ModelSelect_module_css_default.menu,
						style: flyoutMode ? flyoutStyle : menuPos ?? MEASURE_STYLE,
						role: "menu",
						onMouseEnter: flyoutMode ? () => {
							clearFlyoutClose();
						} : void 0,
