window.__ModuleLoader__.load({
 ...
 /* v8 ignore next 4 -- plain-text composition nests no block elements today; the walk stays total for imported states. */
		...
		* moves the New Session flow to that workspace's blank session. A session
		* that already has content stays put; only its live cwd follows the pick.
		* Without a
		...
		* DragHandle capture model. */
		...
		function ConversationRoot({ sessionId, useSession, useSessions, useSessionPendingInteraction, useWorkspaces, useConversation, useInput, useComposerBlock, renderSlot, renderSlotChain, selectWorkspace, t }) {
			...
			// The workspace chip and the Agent-preset seat stay on screen for the
			// whole session, not just on the hero. They were hero-only, so the
			// workspace and mode controls vanished the moment a task started —
			// which is exactly when switching workspace is still useful. Gated on
			// the shell's marker script: the browser, which loads this same
			// bundle, keeps the stock layout.
			const keepWorkspaceRow = hero || typeof window !== "undefined" && window.__DSH_SIDEBAR_ACTIONS__ === true;
		...
		function apply(ctx, config = Config({})) {
			...
			const registerConversationRoot = () => slots.register({
				...
				inject: (sessionId) => ({
					...
					}, { reuseCurrent: current !== void 0 && current.blank !== true });
					}
				})
			}, ConversationRoot);
