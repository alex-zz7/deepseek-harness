...
The Session row's Rename action opens a dialog prefilled with the row's display title; confirming an unchanged title is deliberately allowed — it pins the current automatic title against regeneration. Archive opens a confirmation that names the Session and states the retention boundary; when the Session is still running (or has running subagents), the dialog says so and its confirm button becomes **Stop and archive** — confirming stops the in-flight turn first and then archives, so a running chat is never hidden with its run still going. An idle Session archives directly on confirm. The row disappears from every grouping surface when the archive-set echo lands. The Ungrouped bucket's Clear confirmation carries the same running count for the chats it is about to archive. Fork forks at the source's last completed turn, increments the inherited persisted title on the client, and then opens the child. Workspace Delete opens a confirmation that states the retention boundary; success removes the group while its Sessions remain under Ungrouped.
