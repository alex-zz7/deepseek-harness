window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Point the open session at another folder without minting a blank one. */
		async function rehomeOpenSession(workspaceId, sessionId) {
			try {
				const response = await fetch("/sidebar-editor/rehome", {
					method: "POST",
				...
				if (!response.ok || body?.ok !== true) {
					console.warn("workspace rehome failed:", body?.message ?? response.status);
				}
			} catch (reason) {
		...
		var UiWorkspaceService = class extends _deepseek_ai_cordis.Service {
			...
			*/
			...
			async openWorkspace(workspaceId, beforeOpen, opts) {
				const navigation = AbortSignal.any([this.ctx.layout.beginNavigation(), this.lifetime.signal]);
				const isCurrent = () => !navigation.aborted;
				if (opts?.reuseCurrent === true) {
					const snapshot = this.sessions.list.getSnapshot();
					const currentId = snapshot.current;
					const current = currentId === void 0 ? void 0 : snapshot.byId[currentId];
					if (current !== void 0 && current.blank !== true) {
						await rehomeOpenSession(workspaceId, currentId);
						if (!isCurrent()) return;
						beforeOpen?.(currentId);
						if (isCurrent()) this.openSession(currentId);
						return;
					}
				}
		...
		*/
		...
		/**
		* Synthetic group key for the app-only Pinned section.
		*
		* A NUL prefix keeps it out of the workspace-id and ungrouped-key spaces,
		* so it can never collide with a real group.
		*/
		const PINNED_GROUP_KEY = "\u0000pinned";
		/**
		* Read the pinned ids the app injected.
		*
		* Returns empty unless the native shell's user script ran, which is what
		* keeps this section app-only: the browser loads the same bundle but never
		* receives the flag, so it renders exactly as shipped.
		...
		*
		* A pinned section is prepended when the app supplied pinned ids. Its
		* sessions are lifted OUT of their own workspaces, which is the point: a
		* pinned session has to stay reachable while its workspace is collapsed,
		* and `groupByWorkspace` returns no rows for a collapsed group.
		*
		...
		*/
		function deriveGroups(list, workspaces, archivedSessionIds, pendingInteractions, view, pinnedIds, vaults) {
			...
			const groups = [];
			const wanted = Array.isArray(pinnedIds) ? pinnedIds.filter((id) => !archived.has(id)) : [];
			if (wanted.length > 0) {
				const members = [];
				for (const id of wanted) {
					if (members.some((member) => member.id === id)) continue;
					const session = list.byId[id];
					if (session !== void 0) members.push(session);
				}
				if (members.length > 0) {
					groups.push({
			...
			for (const g of vaults?.length ? groupByVaults(list, vaults, workspaces, archived) : groupByWorkspace(list, workspaces, archived, view.ungroupedOrder)) {
				...
				groups.push({
					...
					sessions: expanded ? own.map((session) => sessionNode(session, descendants, pendingInteractions)) : []
				});
			}
			return groups;
		}
		...
		*/
		/** Pin glyph for the Pinned group header; the primitives set ships none. */
		function PinnedGroupIcon() {
		...
		function ProjectRowItem({ group, onToggle, onCreate, actions, drag, home, t }) {
			const row = group;
			// A synthetic group carries its own label; only a real workspace or the
			// ungrouped bucket falls back to the product's text.
			...
			const ownRow = (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					(0, react_jsx_runtime.jsx)("span", {
						...
						children: row.pinned === true ? (0, react_jsx_runtime.jsx)(PinnedGroupIcon, {}) : row.expanded ? (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconFolderOpen16, {}) : (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconFolderClose16, {})
					}),
					(0, react_jsx_runtime.jsx)("span", {
		...
		*/
		...
		const cursorPickerCss = `.dsh-cursor-picker{--dsh-cp-border:var(--dsw-alias-border-l2,rgba(0,0,0,.08));--dsh-cp-hover:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.12));position:fixed;z-index:2147483000;isolation:isolate;display:flex;align-items:flex-end;filter:drop-shadow(0 16px 40px rgba(0,0,0,.16));font:13px/1.35 -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif;color:var(--dsw-alias-label-primary,#111)}.dsh-cursor-pane{box-sizing:border-box;background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsh-cp-border);display:flex;flex-direction:column;min-height:0;max-height:inherit;overflow:hidden}.dsh-cursor-pane-left{width:280px;border-radius:12px}.dsh-cursor-picker[data-flyout] .dsh-cursor-pane-left{border-top-right-radius:0;border-bottom-right-radius:0}.dsh-cursor-pane-right{width:260px;border-left:none;border-radius:0 12px 12px 0}.dsh-cursor-head{display:flex;align-items:center;gap:8px;padding:10px 12px 8px;flex:none}.dsh-cursor-head-title{min-width:0;flex:1;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dsh-cursor-head-meta{flex:none;color:var(--dsw-alias-label-tertiary,#888);font-size:12px}.dsh-cursor-search{margin:0 10px 8px;padding:7px 10px;border:none;border-radius:8px;background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.1));color:inherit;font:inherit;outline:none;width:calc(100% - 20px);box-sizing:border-box;flex:none}.dsh-cursor-search::placeholder{color:var(--dsw-alias-label-tertiary,#999)}.dsh-cursor-scroll{flex:1 1 auto;min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;padding:0 6px 8px;-webkit-overflow-scrolling:touch}.dsh-cursor-scroll::-webkit-scrollbar{width:8px}.dsh-cursor-scroll::-webkit-scrollbar-thumb{background:rgba(127,127,127,.35);border-radius:999px}.dsh-cursor-section{color:var(--dsw-alias-label-tertiary,#888);font-size:11px;font-weight:600;padding:8px 8px 4px}.dsh-cursor-row{display:flex;align-items:center;gap:8px;width:100%;padding:6px 8px;border:none;border-radius:8px;background:transparent;color:inherit;font:inherit;text-align:left;cursor:pointer}.dsh-cursor-row:hover,.dsh-cursor-row[data-active="1"]{background:var(--dsh-cp-hover)}.dsh-cursor-row svg{flex:none}.dsh-cursor-row-label{min-width:0;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dsh-cursor-chevron{flex:none;color:var(--dsw-alias-label-tertiary,#999)}.dsh-cursor-check{flex:none;width:14px;text-align:center;color:var(--dsw-alias-state-business-primary,#4d6bfe)}.dsh-cursor-toggle{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 10px 8px;flex:none}.dsh-cursor-switch{appearance:none;width:32px;height:18px;border:none;border-radius:999px;background:rgba(127,127,127,.28);position:relative;cursor:pointer;flex:none}.dsh-cursor-switch:after{content:"";position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:#fff;transition:transform .15s ease}.dsh-cursor-switch[data-on="1"]{background:#4d6bfe}.dsh-cursor-switch[data-on="1"]:after{transform:translateX(14px)}.dsh-cursor-status{padding:10px 12px;color:var(--dsw-alias-label-tertiary,#888);font-size:12px}.dsh-cursor-footer{padding:8px 10px;border-top:1px solid var(--dsh-cp-border);flex:none}.dsh-cursor-footer button{width:100%;padding:7px 10px;border:none;border-radius:8px;background:#4d6bfe;color:#fff;font:inherit;cursor:pointer}`;
		if (typeof document !== "undefined") {
			let tag = document.querySelector("style[data-plugin-css=\"dsh-cursor-picker\"]");
			if (tag === null) {
				tag = document.createElement("style");
				tag.dataset.pluginCss = "dsh-cursor-picker";
				document.head.appendChild(tag);
			}
			tag.textContent = cursorPickerCss;
		}
		function CursorGlyph({ path, size = 16 }) {
		...
		const ICON_EXISTING = "M3 4.5h10v8H3zM3 7h10";
		const ICON_CHEVRON = "M6 4.5 10 8 6 11.5";
		function CursorRow({ icon, label, chevron, active, checked, onClick, onPointerEnter }) {
			return (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				className: "dsh-cursor-row",
				"data-active": active ? "1" : undefined,
				onClick,
				onPointerEnter,
				children: [
					(0, react_jsx_runtime.jsx)(CursorGlyph, { path: icon }),
					(0, react_jsx_runtime.jsx)("span", { className: "dsh-cursor-row-label", children: label }),
					checked ? (0, react_jsx_runtime.jsx)("span", { className: "dsh-cursor-check", children: "✓" }) : null,
					chevron ? (0, react_jsx_runtime.jsx)("span", { className: "dsh-cursor-chevron", children: (0, react_jsx_runtime.jsx)(CursorGlyph, { path: ICON_CHEVRON, size: 12 }) }) : null
				]
			});
		}
		function CursorWorkspaceMenu({ open, getAnchorRect, anchorRef, side = "top", workspaces, selectedId, currentTitle, createWorkspace, onPick, onClose, openDirectoryFlow }) {
			...
			const [multi, setMulti] = (0, react.useState)(false);
			const [picked, setPicked] = (0, react.useState)(() => new Set());
			...
			(0, react.useEffect)(() => {
				if (!open || flyout !== "remote") return;
				let cancelled = false;
				setStatus("正在读取 GitHub 仓库…");
				fetch("/sidebar-editor/github-repos").then((response) => response.json()).then((body) => {
					if (cancelled) return;
					if (!body?.ok) {
						setRepos([]);
						setStatus(body?.message || "读取 GitHub 失败");
						return;
					}
					setRepos(body.repos ?? []);
					setStatus(body.repos?.length ? "" : "GitHub 账号下没有仓库");
				}).catch((error) => {
					if (!cancelled) {
						setRepos([]);
			...
			(0, react.useEffect)(() => {
				if (!open || flyout !== "local") return;
				let cancelled = false;
				setStatus("正在读取本机文件夹…");
				fetch("/sidebar-editor/local-folders").then((response) => response.json()).then((body) => {
					if (cancelled) return;
					setFolders(body.folders ?? []);
					setStatus(body.folders?.length ? "" : "~/Projects 还是空的");
				}).catch((error) => {
					if (!cancelled) setStatus(String(error?.message ?? error));
				});
				return () => {
					cancelled = true;
				};
			}, [open, flyout]);
			const adopt = (0, react.useCallback)((path) => {
				setBusy(true);
				setStatus("正在打开…");
			...
			const requestNativeFolder = (0, react.useCallback)((mode) => {
				const handler = window.webkit?.messageHandlers?.dshPickWorkspace;
				if (handler === undefined) {
					setStatus("这个操作只能在 App 里用");
					return;
				}
				const create = createWorkspace;
				const pick = onPick;
				window.__dshAdoptWorkspace = (path) => {
					window.__dshAdoptWorkspace = undefined;
					if (typeof path !== "string" || path === "") return;
					create({ path }).then((workspace) => pick(workspace.workspaceId)).catch((error) => {
						console.warn("adopt workspace failed:", error);
					});
				};
				onClose();
				handler.postMessage(mode);
			}, [createWorkspace, onPick, onClose]);
			if (!open) return null;
			const needle = query.trim().toLowerCase();
			const recent = workspaces.filter((workspace) => {
				if (!needle) return true;
				return workspace.title.toLowerCase().includes(needle) || (workspace.path ?? "").toLowerCase().includes(needle);
			});
			const remoteNeedle = remoteQuery.trim().toLowerCase();
			const visibleRepos = repos.filter((repo) => {
				if (!remoteNeedle) return true;
				return `${repo.owner}/${repo.name}`.toLowerCase().includes(remoteNeedle);
			});
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								className: "dsh-cursor-head",
								children: [
									(0, react_jsx_runtime.jsx)(CursorGlyph, { path: ICON_FOLDER }),
									(0, react_jsx_runtime.jsx)("span", { className: "dsh-cursor-head-title", children: currentTitle || "工作区" }),
									(0, react_jsx_runtime.jsx)("span", { className: "dsh-cursor-head-meta", children: "这台 Mac" })
								]
							}),
							(0, react_jsx_runtime.jsx)("input", {
								className: "dsh-cursor-search",
								value: query,
								placeholder: "搜索仓库、本机文件夹…",
								onChange: (event) => setQuery(event.target.value)
							}),
							(0, react_jsx_runtime.jsxs)("div", {
								className: "dsh-cursor-scroll",
								onWheel: stopMenuWheel,
								children: [
									(0, react_jsx_runtime.jsx)("div", { className: "dsh-cursor-section", children: "最近" }),
									recent.length === 0 ? (0, react_jsx_runtime.jsx)("div", { className: "dsh-cursor-status", children: "还没有最近的工作区" }) : recent.map((workspace) => (0, react_jsx_runtime.jsx)(CursorRow, {
										icon: ICON_FOLDER,
									...
									(0, react_jsx_runtime.jsx)("div", { className: "dsh-cursor-section", children: "仓库" }),
									(0, react_jsx_runtime.jsx)(CursorRow, {
										icon: ICON_FOLDER,
										label: "本机",
										chevron: true,
										active: flyout === "local",
										onPointerEnter: () => setFlyout("local"),
										onClick: () => setFlyout("local")
									}),
									(0, react_jsx_runtime.jsx)(CursorRow, {
										icon: ICON_CLOUD,
										label: "远程",
										chevron: true,
										active: flyout === "remote",
										onPointerEnter: () => setFlyout("remote"),
										onClick: () => setFlyout("remote")
									}),
									(0, react_jsx_runtime.jsx)(CursorRow, {
										icon: ICON_PLUS,
										label: "从零开始",
										onClick: () => requestNativeFolder("scratch")
									}),
									(0, react_jsx_runtime.jsx)(CursorRow, {
										icon: ICON_EXISTING,
										label: "使用已有…",
									...
									(0, react_jsx_runtime.jsx)(CursorRow, {
										icon: ICON_FOLDER,
										label: "新建文件夹",
										onClick: () => requestNativeFolder("new-folder")
									})
								]
							})
						]
					}),
					flyout ? (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-cursor-pane dsh-cursor-pane-right",
						...
						children: [
							(0, react_jsx_runtime.jsx)("input", {
								...
								style: { marginTop: 10 },
								value: remoteQuery,
								placeholder: flyout === "remote" ? "搜索远程仓库…" : "搜索本机文件夹…",
								onChange: (event) => setRemoteQuery(event.target.value)
							}),
							flyout === "remote" ? (0, react_jsx_runtime.jsxs)("div", {
								className: "dsh-cursor-toggle",
								children: [
									(0, react_jsx_runtime.jsx)("span", { children: "多选" }),
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: "dsh-cursor-switch",
										"data-on": multi ? "1" : "0",
										"aria-pressed": multi,
							...
							(0, react_jsx_runtime.jsxs)("div", {
								...
								onWheel: stopMenuWheel,
								children: [
									status ? (0, react_jsx_runtime.jsx)("div", { className: "dsh-cursor-status", children: status }) : null,
									flyout === "remote" ? visibleRepos.map((repo) => {
										...
										label: folder.name,
										onClick: () => {
											if (!busy) adopt(folder.path);
										}
									}, folder.path))
								]
							}),
							flyout === "remote" && multi && picked.size > 0 ? (0, react_jsx_runtime.jsx)("div", {
								className: "dsh-cursor-footer",
								children: (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									disabled: busy,
									onClick: cloneSelected,
									children: `打开 ${picked.size} 个仓库`
								})
							}) : null
						]
					}) : null
		...
		/**
		* App-only menu row: clone one of the user's GitHub repositories and
		* register it as a workspace. The browsing and cloning happen in the
		* native shell (`ProjectPickerController`), because a web menu cannot own
		* a search field over `gh repo list` and a clone that runs for minutes.
		*
		* Gated on the marker script the shell injects, so the browser — which
		* loads this very bundle — never grows an entry that cannot work there.
		*/
		const GITHUB_CLONE = "::github-clone";
		/**
		...
		*/
		function WorkspacePickFlow({ t, open, anchorRef, useWorkspaces, createWorkspace, useDirectoryFlow, renderDirectoryFlow, onPick, onClose, addOnly = false, side = "bottom", selectedId }) {
			...
			const handleSelect = (id) => {
				...
				if (id === GITHUB_CLONE) {
					// The native panel does the browsing, cloning and registering;
					// it ends by registering the clone through the plugin route, so
					// the new workspace arrives over the same follow stream.
					onClose();
					window.webkit?.messageHandlers?.dshOpenProjects?.postMessage("open");
					return;
				}
				onPick(id);
			};
			const currentTitle = workspaces.find((workspace) => workspace.workspaceId === selectedId)?.title ?? workspaces[0]?.title ?? "";
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				appOnly ? (0, react_jsx_runtime.jsx)(CursorWorkspaceMenu, {
					open,
					getAnchorRect,
					anchorRef,
					side,
		...
		*/
		...
		function SessionTree({ useSessions, useSessionPendingInteraction, startSession, open, forkSession, workspaces: allWorkspaces, archivedSessionIds, workspaceReady, usePanelInfo, onRenameRequest, onDeleteRequest, onSessionRename, onSessionArchive, insertWorkspaceBefore, insertSessionBefore, orderBy, groupExpansion, setGroupExpanded, sessionOrderByAccount, sessionUpdatedAtByAccount, syncSessionOrderAccount, setSessionOrder, home, t, revealSessionId, onSessionRevealed }) {
			...
			// App-only Pinned section. The ids arrive from the native shell's
			// injected script through a window event, so a browser — which never
			// receives that script — keeps `pinnedIds` empty and renders as shipped.
			const [pinnedIds, setPinnedIds] = (0, react.useState)(readPinnedIds);
			(0, react.useEffect)(() => {
				const onPinnedChange = (event) => {
					const ids = event?.detail?.ids;
					setPinnedIds(Array.isArray(ids) ? ids : readPinnedIds());
				};
				window.addEventListener("dsh-pins-changed", onPinnedChange);
				return () => window.removeEventListener("dsh-pins-changed", onPinnedChange);
			}, []);
